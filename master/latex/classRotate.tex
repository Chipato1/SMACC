...
\begin{DoxyCode}
26   \{
27     \hyperlink{classRotate_a0d494e54d8311aafe97bd44b381c039c}{rotateDegree} = rotate\_degree;
28   \}
\end{DoxyCode}
...
62         ROS\_ERROR(\textcolor{stringliteral}{"%s"},ex.what());
63         ros::Duration(1.0).sleep();
64         \}
65     \}   
66 
67     smacc::SmaccMoveBaseActionClient::Goal goal;
68     goal.target\_pose.header.frame\_id = \textcolor{stringliteral}{"/odom"};
69     goal.target\_pose.header.stamp = ros::Time::now();
70 
71     \textcolor{keyword}{auto} currentAngle = tf::getYaw(currentPoseMsg.orientation);
72     \textcolor{keyword}{auto} targetAngle = currentAngle + angle\_increment\_degree *M\_PI/180.0;
73     goal.target\_pose.pose.orientation =tf::createQuaternionMsgFromYaw(targetAngle);
74     goal.target\_pose.pose.position = currentPoseMsg.position;
75     ROS\_WARN\_STREAM(\textcolor{stringliteral}{"rotate behavior current\_state : "} << currentPoseMsg);
76     ROS\_WARN\_STREAM(\textcolor{stringliteral}{"rotate behavior target : "} << goal.target\_pose.pose);
77 
78     ROS\_INFO(\textcolor{stringliteral}{"forward behavior: sending forward goal"});
79     \hyperlink{classRotate_ae632332fc9901b146697d5942d83ed42}{moveBaseClient\_}->\hyperlink{classsmacc_1_1SmaccActionClientBase_a58c67a87c5fb8ea1633573c58fe3eee1}{sendGoal}(goal);
80   \}    
\end{DoxyCode}
